...
void *ShmAllocator::shm_alloc(size_t size)
{
    ...
    TESTPRINT("looking for available key to allocate against");
    while (true) {
        current_key &= 1; // -2 becomes 0
        if (used[current_key].try_lock()) {   // if key is available, break and allocate against this key
            break;
        } else { // if current key locked, toggle it and try the other key
            current_key ^= 1;
            // try locking other key
            if (used[current_key].try_lock()) {
                break;
            }
            // else return malloc(size); // allocate from heap memory if both keys used (can be activated for efficiency of the simulation
        }
    }
    ...
    if ((shmids[current_key] = shmget(sems[current_key], size, 0666|IPC_CREAT)) == -1) {
        TESTPRINT("shmget error in producer");
...
//    out = std::async(std::launch::async, &ShmAllocator::wait_del, this, key);
    std::thread f(&ShmAllocator::wait_del, this, key);
    f.detach();
    TESTPRINT("Returning from free\n");
}
...
void ShmAllocator::wait_del(int key)
{
    ...
    used[key].unlock();
    TESTPRINT("Producer has deleted and unlocked key %d\n", key);
}